...
# 간단한 등급용 총점
# 점수 분해는 출력에서 제거하고 내부 등급용으로만 유지
W_CLOSE_TIGHTNESS = 40.0
W_VOLATILITY_CONTRACTION = 35.0
W_VOLUME_DRY_UP = 25.0
...
def build_summary_message(regime: MarketRegime, radar_df: pd.DataFrame) -> str:
    ...
    for i in range(min(len(radar_df), TOP_OUTPUT)):
        ...
        lines.append(
            f"등급 {row['grade']} | RS근접 {row['rs_near_ratio']*100:.1f}% | 전고근접 {row['price_near_ratio']*100:.1f}%"
        )
        lines.append(
        ...
        lines.append(
            f"종가범위 {row['close_range_pct_10d']:.2f}% | 변동성비 {row['volatility_ratio_10v30']:.3f} | "
            f"거래량비 {row['volume_ratio_10v30']:.3f}"
        )
        lines.append(
            f"50일거리 {row['sma50_distance_ratio']:.3f} | 상단유지 {int(row['band_hold_days'])}/{int(row['band_elapsed_days'])}"
        )
