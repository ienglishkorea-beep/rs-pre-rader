...
MIN_MARKET_CAP = float(os.getenv("MIN_MARKET_CAP", "1000000000"))  # 10억 달러
MIN_DOLLAR_VOLUME_21D = float(os.getenv("MIN_DOLLAR_VOLUME_21D", "12000000"))  # 1200만 달러
...
PRICE_NEAR_RATIO_MIN = float(os.getenv("PRICE_NEAR_RATIO_MIN", "0.95"))  # 55일 전고점의 95% 이상
...
W_RS_NEAR = 30.0
W_PRICE_NEAR = 25.0
W_CLOSE_TIGHTNESS = 15.0
W_VOLATILITY_CONTRACTION = 15.0
W_VOLUME_DRY_UP = 15.0
...
def calc_radar_row(
    ...
    vol_long = calc_avg_daily_range_pct(df.iloc[:-VOLATILITY_SHORT_DAYS], VOLATILITY_LONG_DAYS)
    ...
    # 점수화
    score_rs_near = linear_score_high_better(
        rs_near_ratio, 0.95, 1.00, W_RS_NEAR
    )

    score_price_near = linear_score_high_better(
        price_near_ratio, 0.95, 1.00, W_PRICE_NEAR
    )

    # 종가 범위는 낮을수록 좋다.
    # 2% 이내 최상, 10% 이상은 거의 0점
    score_close_tightness = linear_score_low_better(
        close_range_pct_10d, 2.0, 10.0, W_CLOSE_TIGHTNESS
    )

    # 변동성 비율은 0.55~1.00을 널널하게 사용
    score_volatility_contraction = linear_score_low_better(
        volatility_ratio, 0.55, 1.00, W_VOLATILITY_CONTRACTION
    )

    # 거래량 축소도 0.50~1.00을 널널하게 사용
    ...
    return RadarRow(
        ...
        score_rs_near=round(score_rs_near, 2),
        score_price_near=round(score_price_near, 2),
...
def build_summary_message(regime: MarketRegime, radar_df: pd.DataFrame) -> str:
    ...
    for i in range(min(len(radar_df), TOP_OUTPUT)):
        ...
        lines.append(
            f"점수  RS {row['score_rs_near']:.1f}/30 | 전고 {row['score_price_near']:.1f}/25 | "
            f"종가압축 {row['score_close_tightness']:.1f}/15"
        )
        lines.append(
            f"      변동성축소 {row['score_volatility_contraction']:.1f}/15 | "
            f"거래량축소 {row['score_volume_dry_up']:.1f}/15"
        )
