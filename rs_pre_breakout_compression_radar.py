...
PRICE_NEAR_RATIO_MIN = float(os.getenv("PRICE_NEAR_RATIO_MIN", "0.93"))  # 전고점 93% 이상
RS_NEAR_RATIO_MIN = float(os.getenv("RS_NEAR_RATIO_MIN", "0.95"))        # RS 95% 이상
...
TOP_STAY_DAYS = int(os.getenv("TOP_STAY_DAYS", "10"))
TOP_STAY_RATIO = float(os.getenv("TOP_STAY_RATIO", "0.97"))  # pivot의 97% 이상 구간에 머문 날

# 점수 비중
# 입장권은 필터로만 쓰고, 점수는 압축/축소 + 추가점수 위주
...
class RadarRow:
    ...
    top_stay_count_10d: int
    top_stay_ratio_10d: float
...
def linear_score_mid_best(
    value: float, best_low: float, best_high: float, bad_low: float, bad_high: float, max_points: float
) -> float:
    """
...
def calc_top_stay_count(df: pd.DataFrame, pivot_55: float, days: int, threshold_ratio: float) -> int:
    part = df.tail(days).copy()
    if len(part) < days or pd.isna(pivot_55) or pivot_55 <= 0:
        return 0
    threshold = pivot_55 * threshold_ratio
    return int((part["Close"] >= threshold).sum())
...
def calc_radar_row(
    ...
    top_stay_count = calc_top_stay_count(df, pivot_55, TOP_STAY_DAYS, TOP_STAY_RATIO)
    top_stay_ratio_10d = safe_div(top_stay_count, TOP_STAY_DAYS, default=0.0)
    ...
    # 상단 체류 시간: 최근 10일 중 pivot의 97% 이상 구간에 며칠 머물렀는지
    score_top_stay = linear_score_high_better(
        top_stay_count, 2, TOP_STAY_DAYS, W_TOP_STAY
    )
    ...
    return RadarRow(
        ...
        top_stay_count_10d=top_stay_count,
        top_stay_ratio_10d=round(top_stay_ratio_10d, 3),
        ...
        score_top_stay=round(score_top_stay, 2),
...
def build_summary_message(regime: MarketRegime, radar_df: pd.DataFrame) -> str:
    ...
    for i in range(min(len(radar_df), TOP_OUTPUT)):
        ...
        lines.append(
            ...
            f"상단체류 {int(row['top_stay_count_10d'])}/{TOP_STAY_DAYS}"
        )
        lines.append(
        ...
        lines.append(
            ...
            f"상단체류 {row['score_top_stay']:.1f}/10"
        )
